...
$ahoy_log = '/tmp/AhoyDTU_80247680.log';
...
fseek($ahoy_fh, -120, SEEK_END);	// Zum Ende der Datei springen, um nur neue Einträge zu lesen
fseek($nginxfh, -121, SEEK_END);	// Zum Ende der Datei springen, um nur neue Einträge zu lesen

while (true) {
	// read line an print it to client
	while (($ahoy_line = fgets($ahoy_fh)) !== false) print2client($ahoy_line);
	// reset file pos
	if (filesize($ahoy_log) > ftell($ahoy_fh)) fseek($ahoy_fh, ftell($ahoy_fh));

	// read line an print it to client
	while (($nginxline = fgets($nginxfh)) !== false) print2client($nginxline);
	// reset file pos
